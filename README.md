...
Zike is a fully-featured Discord bot with a modern web dashboard for managing everything from antinuke to music. Built on `discord.py v2`, `FastAPI`, and `Next.js 14` with Tailwind CSS.

```
Zike-AIO-With-Dashboard/
...
- Arsh watermark on every source file
...
```bash
git clone https://github.com/Arsh/Zike-AIO-With-Dashboard
cd Zike-AIO-With-Dashboard/bot
```
...
NEXT_PUBLIC_BRAND_NAME_WORD   = "ZK"
```
...
2. Choose **Cloudflared**, give it a name (e.g. `zike-api`), save
...
2. Set the start command to `python Zike.py`
